...
print("Model:", 'models/pretr_Linf.pth')
...
print("Model:", 'models/pretr_L2.pth')
test_model_on_single_attack(model, attack='pgd_linf', eps=8/255)
...
print("Model:", 'models/pretr_RAMP.pth')
test_model_on_single_attack(model, attack='pgd_linf', eps=8/255)
...
#
# Multi-Norm Robust Accuracy Evaluation
# 
def test_model_on_multi_attacks(model, eps_linf=8./255., eps_l2=0.75):
    ...
    for batch_idx, (x_batch, y_batch) in tqdm(enumerate(test_loader), total=len(test_loader), desc="Evaluating"):
        ...
        x_adv_linf = pgd_linf_untargeted(model, x_batch, y_batch, k=10, eps=eps_linf, eps_step=eps_linf/4.0)
        x_adv_l2 = pgd_l2_untargeted(model, x_batch, y_batch, k=10, eps=eps_l2, eps_step=eps_l2/4.0)

        ## calculate union accuracy: correct only if both attacks are correct
        
        with torch.no_grad():
            out = model(x_adv_linf)
            pred_linf = torch.argmax(out, dim=1)
            out = model(x_adv_l2)
            pred_l2 = torch.argmax(out, dim=1)
        
            # Get the testing accuracy with multi-norm robustness and update tot_test and tot_acc
            correct_both = ((pred_linf == y_batch) & (pred_l2 == y_batch)).sum().item()
            tot_acc += correct_both
            tot_test += y_batch.size(0)
...
# Evaluate standard accuracy with different models
model.load_state_dict(torch.load('models/pretr_Linf.pth', map_location=device))
# Evaluate standard accuracy with model 1
print("Model:", 'models/pretr_Linf.pth')
test_model_standard_accuracy(model)

model.load_state_dict(torch.load('models/pretr_L2.pth', map_location=device))
# Evaluate standard accuracy with model 2
print("Model:", 'models/pretr_L2.pth')
test_model_standard_accuracy(model)

model.load_state_dict(torch.load('models/pretr_RAMP.pth', map_location=device))
# Evaluate standard accuracy with model 3
print("Model:", 'models/pretr_RAMP.pth')
test_model_standard_accuracy(model)